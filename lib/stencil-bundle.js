...
Bundle.prototype.getCssAssembleTask = function (compiler) {
    ...
    return function (callback) {
        ...
        Fs.readdir(basePath, function (err, files) {
            ...
            Async.map(filterFiles, function (file, mapCallback) {
                ...
                _.each(filterFiles, function (file, index) {
...
Bundle.prototype.assembleLangTask = function (callback) {
    ...
    }, this.configuration.settings.locale);
};
...
function bundleTaskRunner(tasks, callback) {
    ...
    Async.parallel(tasks, function (err, assembledData) {
        ...
        if( self.options.prefix ) {
          try {
            if( fileExists('config.json') ) {
              Fse.copySync( 'config.json', 'config_tmp.json');
              console.log('ok'.green + ' -- old config.json file was backed up');
            }
            Fse.copySync( 'config_' + self.options.prefix + '.json', 'config.json');
            console.log('ok'.green + ' -- config.json file created from prefix');
          } catch( err ) {
            console.error(err);
          }
        }
        ...
        output.on('close', () => {
            ...
            LangAssembler.restoreLangFolder( function() {

              if( self.options.prefix ) {
                try {
                  Fse.removeSync( 'config.json');
                  console.log('ok'.green + ' -- generated config.json file from prefix was removed');
                  if( fileExists('config_tmp.json') ) {
                    Fse.copySync( 'config_tmp.json', 'config.json');
                    console.log('ok'.green + ' -- config.json file restored from backup');
                    Fse.removeSync( 'config_tmp.json');
                    console.log('ok'.green + ' -- config_tmp.json backup file was removed');
                  }
                } catch( err ) {
                  console.error(err);
                }
            ...
            }, self.configuration.settings.locale);
        });
